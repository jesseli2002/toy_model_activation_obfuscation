...
Loads an adversarially-trained checkpoint and produces a report
...
def _auroc_snapshots(
    ...
    for path in paths:
        ...
        if "probe_w" not in ck:
            continue  # not a train_adversarial_logreg.py checkpoint (other
            # checkpoints have no probe_w/probe_b/probe_layers)
...
def main(args):
    ...
    # training script with no adversarial penalty) -- that's the only case
    # that falls back to a default. Once adv_config exists, every field on
